...
checkpoint_valid = ModelCheckpoint('model/model_valid_ner.h5', verbose=1, monitor='val_loss', save_best_only=True, mode='auto')
model_valid = Sequential()
model_valid.add(Bidirectional(LSTM(128, return_sequences=False)))
model_valid.add(Dense(1))

checkpoint_train = ModelCheckpoint('model/model_train_ner.h5', verbose=1, monitor='val_loss', save_best_only=True, mode='auto')
model_train = Sequential()
model_train.add(Bidirectional(LSTM(128, return_sequences=False)))
model_train.add(Dense(1))

def plot(type):
    y = (data['' + type+ ' loss'].rolling(window=5, min_periods=1).mean()).tolist()
    ...
    if type == 'Valid':
        ...
        return
    y_pred_plot = y + predict_new_data_from_pretrained(model, type, predict_count = 100)
...
async def follow(thefile):
    ...
    while True:
        ...
        if not line:
            ...
            continue
        train = re.findall(r'Train Loss = (\d+\.\d+)', line)
        valid = re.findall(r'Valid Loss = (\d+\.\d+)', line)
        if train and valid:
            yield float(train[-1]), \
                  float(valid[-1])
...
async def main(path):
    ...
    async for train_loss, valid_loss in follow(open(path, "r" , encoding='utf-8')):
        if train_loss:
            if is_first_operation==True:
                data = pd.DataFrame({"Train loss":train_loss, "Valid loss":valid_loss}, columns=['Train loss', 'Valid loss', 'Elapsed time'], index=['Epoch'])
                suspected_epoch.append(0)
            else:
                data = data.append({"Train loss":train_loss, "Valid loss":valid_loss}, ignore_index=True)
                suspected_epoch.append(0)
